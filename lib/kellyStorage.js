...
var kellyStorage = new Object();
            
    kellyStorage.fields = {                    
        postLink : {type : 'string', default : '__PROTOCOL__//old.reactor.cc/post/__POST_ID__'},
        delay : {type : 'float', default : 0.8},
        timeout : {type : 'float', default : 4},
        scrollOffset : {type : 'int', default : 100},
        scrollDelay : {type : 'float', default : 0.5},
        debug : {type : 'int', default : 0},
    }; 
    
    kellyStorage.cfg = false;
    kellyStorage.validateCfgVal = function(key, val) {
         ...
         return KellyTools.val(val, this.fields[key].type);
    }
    
    kellyStorage.getDefaultCfg = function() {
        
        var cfg = {};        
        for (var key in this.fields) cfg[key] = this.fields[key].default;
    ...
    kellyStorage.load = function(callback) {
      ...
      KellyTools.getBrowser().runtime.sendMessage({
            ...
            if (response.item) {
                ...
                handler.cfg = KellyTools.parseJSON(response.item);
            ...
            if (!handler.cfg) {                    
                handler.cfg = handler.getDefaultCfg();
            }
