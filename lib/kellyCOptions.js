...
var KellyCOptions = new Object(); 
    ...
    KellyCOptions.init = function() {
        ...
        KellyTools.setHTMLData(document.getElementById('header'), title + copyright); 
        KellyTools.setHTMLData(document.getElementById('chick-advice'), this.getLoc('chik_advice'));
        ...
        KellyStorage.load(function(cfg) {
            ...
            for (var i = 0; i < KellyStorage.fieldsOrder.length; i++) {
                 ...
                 if (KellyStorage.fieldsOrder[i].indexOf('_/') != -1) {
                     html += '</div></div>';
                     continue;
                 }
            ...
            KellyTools.getElementByClass(handler.page, handler.baseClass + '-save-btn').onclick = function() {
                    ...
                    KellyShowRate.getInstance().callback('onOptionsSave', data);                    
