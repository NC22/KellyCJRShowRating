...
KellyTHComments.highlightComment = function(source, r, hs) {
    try { 
        ...
        var hsl = (r < 0 ? 0 : 80 + Math.ceil(hs * 0.4)) + ', ' + (81 + Math.ceil(hs * 0.19)) + '%, ' + (l < 50 ? 50 : l) + '%'; // 80 - 120%, 81 - 100%, 50 - 100%
...
KellyTHComments.enableTweak = function(context) {
    ...
    KellyStorage.addField('_/additions/_', 'highlightHS', {type : 'int', default : 100});
    
    KellyShowRate.getInstance().callbacks.onInit.push(function(content) {        
        if (!content.cfg.highlightCommentsEnabled) return;
        if (content.cfg.highlightHS < 0 || content.cfg.highlightHS > 100) content.cfg.highlightHS = 100;
