...
function KellyShowRate(cfg) {
    ...
    var css = '\
                ...
                .' + baseClass + '-rating:first-child {\
                    margin-left: 0px;\
                    margin-right: 0px;\
                }\
                .' + baseClass + '-rating.' + baseClass + '-rating-setted-loading {\
                    color: #c2c2c2;\
                }\
                .' + baseClass + '-rating.' + baseClass + '-rating-loading {\
                    animation: ' + baseClass + '-LoaderSpin 2s linear infinite;\
                ...
                .' + baseClass + '-rating.old-reactor-cc-rating {\
                    width: 6px!important;\
                    height: 6px!important;\
                }\
                \
                .' + baseClass + '-rating.' + baseClass + '-rating-retry {\
                    cursor : pointer;\
                }\
                ';
    ...
    function updateCommentsRating(post) {
        ...
        var comments = commentsContainer[0].getElementsByClassName('comment');

        for (var i = 0; i < comments.length; i++) {
            
            var unauthComment = cacheDoc.querySelector('#' + comments[i].id); 
    ...
    function startJob() {
        ...
        var src = job.post.querySelector(selector);
    ...
    function updateRatingState(post, scrollCheckOff) {
        ...
        if (!scrollCheckOff) {              
            ...
            if (loader.classList.contains(baseClass + '-rating-checked') || loader.getBoundingClientRect().top > scrollBottom + handler.cfg.scrollOffset) { // under bottom of screen + 100px
                // handler.log('skip ' + post.id);
                return;
            }            
        }
        ...
        addJob(post, function(unauthDoc, error) {
                ...
                cache.push({post : post, unauthDoc : unauthDoc});
