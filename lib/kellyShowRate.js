/* 
    ...
    @name           kellyShowRate
    @namespace      Kelly
    @description    Show rating of posts and comments for authored users on supported HTML layouts
...
function KellyShowRate(cfg) {
    ...
    var ratingStr = {hidden : '≈0', hiddenPost : '--', error : '??'};
    ...
    var baseClass = KellyShowRateTpl.baseClass, hostClass = location.host.split('.').join('-');
    
    var commentsBlockTimer = {}, updateAllTimer = false;    
    var request = false, requestTick = false;
    
    function isAuth() {        
        return window.location.host == 'catface.ru' || document.getElementById('settings') ? true : false;        
    }   
    ...
    function getRatingNode(rating) {
        ...
        if (rating) {
            ...
            while(node = walk.nextNode()) {  // "Рейтинг : 44.4" или "[+] 44.4 [-]" или [+] -- [-] и т.п.
                if (node && node.nodeType == Node.TEXT_NODE && (node.nodeValue.indexOf('.') != -1 || node.nodeValue.indexOf(ratingStr.hiddenPost) != -1 || node.nodeValue.indexOf(ratingStr.hidden) != -1)) {
                    return node;
                }
            }
        }
    ...
    function getPostRatingNode(el, unauth) {        
        ...
        if (!rating.firstChild) KellyTools.setHTMLData(rating, '<span>' + ratingStr.hiddenPost + '</span>'); // sometimes posts has empty rating elem in some sections
        
        // testing only
        
            if (rating.classList.contains('show-rating-checker-local')) {                
                rating.parentElement.classList.add('active');
                return false;
            }
    ...
    function getValidRatingNodeValue(el) { // currently remove addition information like "Рейтинг : " if exists
        var rating = el && el.nodeValue.indexOf(ratingStr.hidden) == -1 ? el.nodeValue.match(/[+-]?\d+(\.\d+)?/g) : false;
    ...
    function updateCommentsRating(post) {
        ...
        if (unauthComments == false || unauthComments.length <= 0) cacheDoc.empty = true; // comments hidden for unauth
        ...
        for (var i = 0; i < comments.length; i++) {
            ...
            if (unauthCommentRating) {
                ...
                if (cacheDoc.empty) loader.innerText = ratingStr.hidden; else handler.log('fail to detect comment rating ' + comments[i].id);
            }
        }
    ...
    function initRatingBlock(el, type) {
        ...
        if (loader.length > 0) return loader[loader.length-1]; // jras copy post_rating entity, so take last one
    ...
    function startJob() {
        ...
        var url = handler.cfg.postLinkR1.replace('__PROTOCOL__', 'http:').replace('__POST_ID__', getPostId(job.post));
    ...
    function updateRatingState(post, scrollCheckOff) {
        ...
        if (!getPostId(post)) {
            handler.log('cant detect post id', true); handler.log(post);
            return;
        }
        ...
        addJob(post, function(unauthDoc, error) {
                ...
                loader.innerText = loader.classList.contains(baseClass + '-rating-setted-loading') ? loader.innerHTML + ' ' + ratingStr.error : ratingStr.error;                        
    ...
    function updateAllRatingState(e, noDelay) {
        ...
        updateAllTimer = setTimeout(function() {
            ...
            for (var i = 0; i < handler.posts.length; i++) if (handler.posts[i]) updateRatingState(handler.posts[i]);   
    ...
    this.init = function() {
        ...
        if (window.location.host == 'catface.ru') {
            
        } else if (head.innerHTML.indexOf('user_id') == -1 || head.innerHTML.indexOf('moderated_tags') == -1) {
            return;
        }        
